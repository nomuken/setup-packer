...
async function getPackerRecentReleases(): Promise<string[]> {
  ...
  const packer_releases = (await octkit.repos.listReleases({owner: 'hashicorp', repo: 'packer', per_page: 100}))

  const releases: string[] = packer_releases.data.map(el => el.tag_name).filter((el): el is string => el !== "nightly" && Boolean(el)).map(el => {
...
async function renamePackerCLI(packer_cli_directory_path: string): Promise<void> {
